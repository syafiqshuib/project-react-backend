...
const resolvers = {
    Query: {
        lists: async (): Promise<Todo[]> => {
            return await Todo.findAll({ order: [['id', 'ASC']] })
        },
        searchList: async (_: unknown, { task }: { task: string }): Promise<Todo[]> => {
    ...
    Mutation: {
        addList: async (_: unknown, { task }: { task: string }): Promise<Todo> => {
            return Todo.create({ task });
        },
        updateList: async (_: unknown, { id }: { id: number }): Promise<Todo | null> => {
        ...
        editList: async (_: unknown, { id, task }: { id: number, task: string }): Promise<Todo | null> => {
            const todo = await Todo.findByPk(id);
            if (!todo) throw new Error("Todo not found");
